...
> **v2.39.0 변경 사항 (화면 구조 전면 개편)**: **"소스 종류" 라디오 하나에
> 조합을 전부 나열하던 방식을 버리고, "소스 종류"(폴더/개별 파일)와
> "목적지 종류"(원격/로컬-그대로/로컬-압축해제)를 독립된 두 라디오 그룹으로
> 분리했습니다.** 스크린샷과 함께 "이 둘은 원래 다른 축 아니냐"는 지적을
> 받고 동의해서 반영했습니다. 덕분에 이전엔 없던 "개별 파일 → 로컬 압축
> 해제"(`file_extract`) 조합도 자연스럽게 생겼습니다. 자세한 것은 아래
> "소스 종류 × 목적지 종류 (6개 조합)" 절 참고.

> **v2.38.0 변경 사항**: **목적지가 Windows 로컬 경로(`K:\다운로드` 등)로
> 보이면 "폴더 전체"/"개별 압축파일"(원격) 모드를 골라도 자동으로 로컬
> 다운로드 모드로 전환됩니다.** "매번 여러 모드 중 로컬/원격을 직접 골라야
> 하냐, Windows인지 자동 판별 가능한 것 아니냐"는 질문에 대한 답입니다.
> 자세한 것은 아래 "로컬/원격 자동 판별이 되는 경우, 안 되는 경우" 절
> 참고.
...
> 갖춰졌습니다(v2.39.0에서 화면 구조를 "소스 종류 × 목적지 종류" 2축으로
> 다시 정리했습니다 - 위 "소스 종류 × 목적지 종류" 절 참고).
...
- **카테고리탭(사이드바 전체 화면, index.html)**: **"소스 종류"**(폴더/개별
  파일, 2개)와 **"목적지 종류"**(원격/로컬-그대로/로컬-압축해제, 3개) -
  서로 독립된 두 개의 라디오 그룹, 소스(URL/ID), 목적지 경로, [복사 시작]/
  [중단] 버튼, 진행률 바, 실시간 로그.
- 이 플러그인은 `admin_only = True`라 **관리자 계정에만** 사이드바에 노출됩니다.

## 소스 종류 × 목적지 종류 (6개 조합, v2.39.0)

**v2.38.0까지는** "소스 종류" 라디오 하나에 조합 5개(폴더 전체/폴더 전체
로컬/개별 압축파일/개별 파일 로컬/일괄 압축해제)를 전부 나열했습니다.
그런데 이건 사실 **"소스가 폴더냐 파일이냐"**와 **"목적지가 원격이냐
로컬이냐, 로컬이면 압축을 풀 거냐"**라는 **서로 독립된 두 개의 선택**을
하나로 욱여넣은 것이었습니다. 그래서 v2.39.0부터는 화면에도 두 개의 독립된
라디오 그룹으로 나눠서 보여줍니다:

- **소스 종류**: 📁 폴더 / 📦 개별 파일
- **목적지 종류**: ☁️ 구글 드라이브(원격) / 💾 로컬(그대로) / 📂 로컬(압축 해제)

두 축을 조합하면 6가지 경우의 수가 나오고, `source_kind` 값으로는 다음과
같이 매핑됩니다(서버로 보내는 값 자체는 예전과 동일한 여섯 가지라, 기존
`job_state.json`이나 외부 연동과 호환됩니다):

| | ☁️ 원격(구글 드라이브) | 💾 로컬(그대로) | 📂 로컬(압축 해제) |
| :--- | :--- | :--- | :--- |
| 📁 **폴더** | `folder` | `folder_local` | `folder_extract` |
| 📦 **개별 파일** | `file` | `file_local` | **`file_extract`** *(v2.39.0 신규)* |

- **원격(`folder`/`file`)**: 구글 드라이브 **안의** 다른 위치로 서버사이드
  복사합니다(g2g = Google-to-Google, 이 플러그인의 원래 목적).
- **로컬-그대로(`folder_local`/`file_local`)**: 서버(또는 Windows라면 K:\
  같은 로컬 드라이브)의 실제 디스크 경로로 압축 해제 없이 다운로드합니다.
- **로컬-압축해제(`folder_extract`/`file_extract`)**: 서버 로컬 스테이징
  폴더에 먼저 받은 뒤 압축을 풀어서 최종 목적지에 저장합니다.
  `folder_extract`는 소스 폴더 안(하위 폴더 포함)의 zip/cbz **전부**를
  찾아 각각 처리하고 원본 폴더 구조를 유지하는 반면, **새로 추가된
  `file_extract`는 폴더 스캔 없이 지정한 파일 하나만** 다운로드+압축
  해제해서 목적지 폴더에 바로 풀어놓습니다(하위 폴더를 따로 만들지
  않으므로, 결과물을 어느 폴더에 모을지는 목적지 경로로 직접 정합니다).

**목적지 종류가 원격이 아닌데 원격인 것처럼(또는 그 반대로) 헷갈리면**
(예: 목적지에 로컬 경로를 입력한 채 "원격"이 선택된 상태) rclone이 그
경로를 원격 경로 문자열로 오인해서 엉뚱하게 동작합니다(예:
`libgdrive_oauth:K:다운로드/`). 화면의 라벨/플레이스홀더가 선택한 조합에
맞춰 바뀌니 확인하고 고르시면 되고, 아래 절에서 설명하는 자동 전환
(v2.38.0)도 이 실수를 상당 부분 막아줍니다.

## 로컬/원격 자동 판별이 되는 경우, 안 되는 경우 (v2.38.0)

**질문**: "여러 모드 중에 로컬/원격을 매번 직접 골라야 하나? Windows인지
리눅스인지 서버가 이미 알고 있는데, 목적지 입력만 보고 자동으로 판별할
수 있는 것 아닌가?"

**답**: 부분적으로 가능합니다 - 방향에 따라 확신도가 다릅니다.

- **Windows 로컬 경로(드라이브 문자 `K:\...`, `K:/...`, 또는 UNC 경로
  `\\서버\공유\...`)는 100% 확신할 수 있습니다.** rclone 원격 상대경로는
  이런 형태를 절대 쓰지 않기 때문입니다("원격이름:경로"에서 우리가 입력받는
  건 콜론 뒤의 경로 부분뿐이고, 그 부분이 드라이브 문자로 시작할 일은
  없음). 그래서 **이 경우는 실제로 자동 전환합니다**: "폴더 전체"/"개별
  압축파일"(원격) 모드가 선택된 상태에서 목적지에 이런 경로를 입력하면,
  화면에서 즉시 "폴더 전체 로컬로 다운로드"/"개별 파일 로컬로 다운로드"로
  라디오가 바뀝니다. 혹시 화면에서 전환 없이(구버전 캐시, 외부 연동 등)
  요청이 오더라도, 서버(`_start_copy()`)가 같은 판정을 한 번 더 해서
  자동으로 바로잡고, 응답 메시지에 "목적지가 Windows 로컬 경로로 보여
  자동으로 로컬 다운로드 모드로 전환했습니다"라고 알려줍니다.
- **POSIX 절대경로(`/data/...`)는 판별할 수 없습니다.** 문제는 이 플러그인
  자체가 rclone 원격 상대경로도 관례적으로 `/`로 시작하게 만들어놨다는
  점입니다(예: 화면의 "폴더 전체" 플레이스홀더 `/zeepsmember/공유_폴더명`).
  `/data/comics/시리즈명`이라는 문자열 하나만 보고는 "서버 로컬 디스크의
  그 경로"를 말하는 건지 "구글 드라이브 원격 안의 그 경로"를 말하는 건지
  구분할 방법이 없습니다 - 같은 표기를 두 가지 의미로 동시에 쓰고 있기
  때문입니다. 그래서 **이 경우는 자동 전환하지 않고, 사용자가 고른 모드를
  그대로 따릅니다.** 잘못 추측해서 반대로 처리해버리면(로컬인데 원격으로,
  또는 그 반대) 지금 겪으신 문제보다 더 알아채기 어려운 사고가 나기
  때문입니다 - 확신이 없을 땐 추측보다 사용자의 명시적 선택을 존중하는
  편이 안전합니다.

정리하면: **Windows 사용자는 사실상 목적지 종류를 신경 안 써도 됩니다** -
목적지에 `K:\...` 형태만 입력하면 알아서 "로컬(그대로)"로 맞춰집니다.
리눅스에서 로컬 디스크로 받고 싶다면 "목적지 종류"에서 "로컬(그대로)" 또는
"로컬(압축 해제)"을 직접 선택해야 합니다(자동 판별 불가 영역이므로).

구현 위치: `logic.looks_like_windows_local_path()`가 판별 규칙을 갖고
있고, `rclone_g2g_copy._start_copy()`(서버)와 `script.js`의
`looksLikeWindowsLocalPath()`(화면)가 완전히 동일한 규칙을 각자 구현해
이중으로 적용합니다.
...
정리하면, 이제 "소스 종류 × 목적지 종류" 조합으로 위 3가지 시나리오 +
구글 드라이브 내부 이동(원래 이 플러그인의 목적)까지 전부 커버합니다 -
자세한 매핑은 위 "소스 종류 × 목적지 종류 (6개 조합)" 절을 참고하세요.
...
- **📦 개별 압축파일**: 구글 드라이브 **파일** 공유 링크(`.../file/d/ID/view`,
  ...
  모드(목적지 종류=원격) 모두 항상 **rclone 원격의 폴더** 경로를 뜻합니다
  (개별 파일 모드에서도 파일명을 따로 입력할 필요 없음). 목적지 종류를
  로컬로 바꾸면 의미가 달라지므로 위 "소스 종류 × 목적지 종류" 절을
  참고하세요.

동작 방식(v2.39.0 기준 - "소스 종류"와 "목적지 종류" 두 라디오 그룹):

- 소스 입력창에 URL을 붙여넣으면 `script.js`가 URL 패턴(`/folders/` vs
  `/file/d/` 또는 `?id=`)을 보고 **"소스 종류" 라디오만** 자동으로
  맞춰줍니다("목적지 종류"는 URL과 무관하므로 건드리지 않습니다). 슬래시
  없는 순수 ID만 붙여넣은 경우는 자동 전환하지 않으므로, 이 경우는 라디오를
  직접 선택해주세요.
- 라벨/플레이스홀더/안내 문구가 선택한 조합(소스 종류 + 목적지 종류)에
  맞춰 바뀝니다.
- `apply(action="start_copy")` 호출 시 두 라디오 그룹의 선택을 조합한
  `item_data.source_kind`(`"folder"` | `"folder_local"` | `"folder_extract"` |
  `"file"` | `"file_local"` | `"file_extract"`)로 서버에 전달되며, 생략되면
  하위 호환을 위해 `"folder"`로 동작합니다(예전 프론트/외부 연동이 이 필드
  없이 호출해도 기존과 동일하게 작동).
- `logic.py`가 `source_kind`에 따라 `get_folder_id()`/`get_file_id()` 중
  맞는 쪽으로 ID를 추출합니다. `folder`/`folder_local`/`file`/`file_local`은
  `_run_job()`이 처리하고, `folder_extract`/`file_extract`는 각각 전용
  함수(`_run_folder_extract_job()`/`_run_file_extract_job()`)가 처리합니다
  (rclone 프로세스를 여러 번 실행하거나 스테이징을 거치는 다른 흐름이라
  공유 코드를 타지 않음). 로그/취소/강제초기화/디스코드 알림 UI는 여섯
  조합 전부 공통으로 동작합니다.
- 새로고침 시 입력창뿐 아니라 **"소스 종류"/"목적지 종류" 라디오 두 개
  모두** `job_state.json`에 저장된 `source_kind`를 `COMBINED_KIND_MAP`의
  역방향 매핑으로 분해해서 그대로 복원됩니다.

- 복사가 진행 중일 때는 "소스 종류"/"목적지 종류" 라디오가 둘 다 잠깁니다
  (이미 시작된 job에는 중간에 종류를 바꿔도 반영되지 않으므로 혼동 방지).
...
./plugins/data/rclone_g2g_copy/
  ...
  staging/<job_id>/         # file_extract 모드 전용 - 원본 압축파일이 잠깐
                    #  머무르는 곳. 압축 해제까지 성공하면 자동 삭제되고,
                    #  실패하면 원본을 보존하기 위해 남겨둔다.
