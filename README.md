...
> 다운로드 모드로 전환됩니다.** "매번 5개 모드 중 로컬/원격을 직접 골라야
...
모드마다 다르게 뜨니 확인하고 고르시면 되고, **아래 절에서 설명하는
자동 전환(v2.38.0)도 이 실수를 상당 부분 막아줍니다.**

## 로컬/원격 자동 판별이 되는 경우, 안 되는 경우 (v2.38.0)

**질문**: "5개 모드 중에 로컬/원격을 매번 직접 골라야 하나? Windows인지
...
정리하면: **Windows 사용자는 사실상 모드를 신경 안 써도 됩니다** - 목적지에
`K:\...` 형태만 입력하면 알아서 로컬 모드로 맞춰집니다. 리눅스에서 로컬
디스크로 받고 싶다면 "폴더 전체 로컬로 다운로드"/"개별 파일 로컬로
다운로드"를 직접 선택해야 합니다(자동 판별 불가 영역이므로).
