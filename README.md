...
- **카테고리탭(사이드바 전체 화면, index.html)**: 소스 종류(폴더 전체/개별
  파일/폴더 안 압축파일 일괄 압축해제) 라디오, 소스(URL/ID), 목적지 경로,
...
- 소스 입력창에 URL을 붙여넣으면 `script.js`가 URL 패턴(`/folders/` vs
  `/file/d/` 또는 `?id=`)을 보고 라디오를 **자동으로 맞춰줍니다**. 폴더
  URL은 "폴더 전체"/"폴더 안 압축파일 일괄 압축 해제" 두 모드 모두의
  정상 입력이라, 이미 이 둘 중 하나가 선택돼 있으면 그대로 두고(파일
  계열이 선택돼 있을 때만 "폴더 전체"로 전환), 파일 URL도 마찬가지로
  "개별 압축파일"이 이미 선택돼 있으면 그대로 둡니다(사용자의 명시적
...
- `apply(action="start_copy")` 호출 시 `item_data.source_kind`(`"folder"` |
  `"file"` | `"folder_extract"`)로 서버에 전달되며, 생략되면 하위 호환을 위해
  `"folder"`로 동작합니다 (예전 프론트/외부 연동이 이 필드 없이 호출해도
  기존과 동일하게 작동).
- `logic.py`가 `source_kind`에 따라 `get_folder_id()`/`get_file_id()` 중
  맞는 쪽으로 ID를 추출합니다. `folder`/`file`은 `_run_job()`이 처리하고,
  `folder_extract`는 완전히 별도인 `_run_folder_extract_job()`이 처리합니다
  (rclone 프로세스를 여러 번 실행하는 다른 흐름이라 공유 코드를 타지
  않음). 로그/취소/강제초기화/디스코드 알림 UI는 세 모드가 공통으로
  동작합니다.
